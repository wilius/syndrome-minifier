...
public class CSSMinifier {
	...
	protected void compress(String path, Writer out, int linebreakpos) throws IOException {
		Pattern p;
		Matcher m;
		String css = getFileContent(path);
		...
		css = this.preserveToken(css, "progid:DXImageTransform.Microsoft.Matrix",
	...
	public static boolean minify(Path cwd, Project project) throws IOException {
		List<String> pathList = project.getStyle().getPathList();

		System.out.println(String.format("CSS file list as below for project %s", project.getName()));
		for (String file : pathList)
			System.out.println(file);

		String path = project.getStyle().getOutputPath(cwd);
		System.out.println(String.format("\nOutput file for project %s is '%s'", project.getName(), path));
		System.out.println("CSS compiling is in progress");
		System.out.println("Wait for while compiling your styles");

		FileWriter writer = new FileWriter(path);
		CSSMinifier minifier = new CSSMinifier();

		for (String file : pathList)
			minifier.compress(file, writer, -1);
		writer.close();
		System.out.println(String.format("Compile of styles of the project %s completed", project.getName()));
		return true;
	}
}
